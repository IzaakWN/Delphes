...
#                 print "WARNING! D1 PID in [ 12, 14 ]"
        elif abs(particle.PID) == 25 and 0 <= D1 < len(event.particles) and event.particles[D1]:
            event.particle_dict['H bosons'].append(particle)
            if abs(event.particles[D1].PID) == 5: # b-quark
                event.particle_dict['b quarks'].extend([ event.particles[D1], event.particles[particle.D2] ])
